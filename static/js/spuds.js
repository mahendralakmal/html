...
else{$Trumba.logger.error("Ugh - no frames support, how did we get here?");}},_setProfile:function(profile){this._spud.profile=profile;},_onDataSuccess:function(result){this._setProfile(result.profile);this._rebuildIFrame(result.body);return;},_onDataTimeout:function(e){$Trumba.logger.error(this._getIFrameId()+" <b>Timeout Error</b> loading("+this.queryString.toString()+")");this._setErrorHTML();},_onDataFailure:function(e){$Trumba.logger.error(this._getIFrameId()+" <b>Data Failure </b> loading("+this.queryString.toString()+")");this._setErrorHTML();},_writeAndClose:function(doc,html){this._spud._hideBusyImage();doc.write(html);doc.close();}}
...
if(url!="")
gotoUrl.getQueryString().setAt("trumbaEmbed",url);if(openInNewWindow){var closeWindowOnReturn=this.getProperty("closeWindowOnReturn");if(closeWindowOnReturn==null||closeWindowOnReturn=="true"||closeWindowOnReturn==true)
